...
data "coder_workspace_preset" "default" {
  ...
  parameters = {
    ...
    if [ ! -d "realworld-django-rest-framework-angular" ]; then
      git clone https://github.com/Above-Os/realworld-django-rest-framework-angular.git
    else
...
locals {
  preview_agent_name = "main"
  preview_app_slug   = "preview"
  preview_path_prefix = "/@${data.coder_workspace_owner.me.name}/${data.coder_workspace.me.name}.${local.preview_agent_name}/apps/${local.preview_app_slug}"
}
...
resource "coder_agent" "main" {
  ...
  env = {
    ...
    CODER_SERVE_APPS_ON_PATH = "1"
    CODER_AGENT_NAME         = local.preview_agent_name
    CODER_APP_SLUG           = local.preview_app_slug
  }
...
resource "coder_app" "preview" {
  ...
  order        = 0
  healthcheck {
    url       = "http://localhost:${data.coder_parameter.preview_port.value}${local.preview_path_prefix}/"
    interval  = 5
    threshold = 15
  }
}
